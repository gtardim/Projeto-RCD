...
function limparTabela() {
    $('#tableCategoriaPP').DataTable().clear().destroy();
}
...
function validaExclusao(cod_tipo) {
            ...
            $.ajax({
                type: 'POST',
                url: '/CategoriaPP/ValidaExclusao',
                data: {
                    cod_tipo
                },
                success: function (res) {
                    if (res.protocolo == null && res.processo == null) {
                        excluir(cod_tipo);
                    }
                    else {
                        if (res.protocolo != null && res.processo != null) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Existem Processos E Protocolos Com esta Categoria Cadastrada',
                                showConfirmButton: false,
                                timer: 6500
                            });
                        }
                            ...
                            if (res.protocolo != null && res.processo == null) {
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Existem Protocolos Com esta Categoria Cadastrada',
                                    showConfirmButton: false,
                                    timer: 6500
                                });
                                ...
                                if (res.processo != null && res.protocolo == null) {
                                    Swal.fire({
                                        icon: 'error',
                                        title: 'Existem Processos Com esta Categoria Cadastrada',
                                        showConfirmButton: false,
                                        timer: 6500
                                    });
                                }
