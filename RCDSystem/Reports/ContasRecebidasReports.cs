...
{
    public class ContasRecebidasReports : TNEReports
    {
        private int _cliente;
        private DateTime? _data_i;
        private DateTime? _data_f;

        public ContasRecebidasReports(int cliente, string data_i, string data_f)
        {
            Paisagem = false;
            _cliente = cliente;
        ...
        public override void MontaCorpoDados()
        {
            ...
            table.AddCell(getNewCell("Valor Pago", titulo, Element.ALIGN_RIGHT, 10, PdfPCell.BOTTOM_BORDER, preto, fundo));
            ...
            var contas = new DAL.QuitarContasReceberDAL().ObterRecebidasRProt(Cliente, Data_i, Data_f);
            if (contas != null)
            {
                ...
                foreach (var d in contas)
                {
                    ...
                    table.AddCell(getNewCell(d.Datageracao.ToString("dd/MM/yyyy"), font, Element.ALIGN_LEFT, 10, PdfPCell.NO_BORDER));
                    table.AddCell(getNewCell(d.Valorpago.ToString(), font, Element.ALIGN_RIGHT, 10, PdfPCell.NO_BORDER));
                    totalprotocolo = d.Valorpago + totalprotocolo;
            ...
            var contasprocesso = new DAL.QuitarContasReceberDAL().ObterRecebidasRProc(Cliente, Data_i, Data_f);
            if (contasprocesso != null)
            {
                ...
                foreach (var d in contasprocesso)
                {
                    ...
                    table.AddCell(getNewCell(d.Datageracao.ToString("dd/MM/yyyy"), font, Element.ALIGN_LEFT, 10, PdfPCell.NO_BORDER));
                    table.AddCell(getNewCell(d.Valorpago.ToString(), font, Element.ALIGN_RIGHT, 10, PdfPCell.NO_BORDER));
                    totalprocesso = d.Valorpago + totalprocesso;

                }

                celltotal = getNewCell("Total Processo: " + totalprocesso.ToString(), titulo, Element.ALIGN_RIGHT, 10, PdfPCell.NO_BORDER);
                celltotal.Colspan = 7;
                table.AddCell(celltotal);
